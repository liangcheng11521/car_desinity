...
def inference(feature, output_shape, keep_prob, is_train):
    ...
    deconv2 = mf.deconvolution_2d_layer(deconv1_relu, [3, 3, 256, 512], [2, 2], [b, 36, 36, 256], 'VALID', wd,
                                        'deconv2')
    deconv2_relu = mf.add_leaky_relu(deconv2, leaky_param)

    deconv3 = mf.deconvolution_2d_layer(deconv2_relu, [3, 3, 128, 256], [2, 2], [b, 74, 74, 128], 'VALID', wd,
                                        'deconv3')
    deconv3_relu = mf.add_leaky_relu(deconv3, leaky_param)

    deconv4 = mf.deconvolution_2d_layer(deconv3_relu, [3, 3, 64, 128], [2, 2], [b, 149, 149, 64], 'VALID', wd,
                                        'deconv4')
