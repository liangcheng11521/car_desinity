...
TEST_TXT = "file_list/test_list1.txt"

# tf.app.flags.FLAGS 的使用，主要是在用命令行执行程序时，需要传些参数 
FLAGS = tf.app.flags.FLAGS
tf.app.flags.DEFINE_integer('feature_row', 8, '''the feature row''')
tf.app.flags.DEFINE_integer('feature_col', 8, '''the feature col''')
tf.app.flags.DEFINE_integer('feature_cha', 2048, '''the feature channel''')

tf.app.flags.DEFINE_integer('label_row', 299, '''the label row''')
tf.app.flags.DEFINE_integer('label_col', 299, '''the label col''')
tf.app.flags.DEFINE_integer('label_cha', 1, '''the label channel''')

tf.app.flags.DEFINE_integer('num_gpus', 1, '''the number of gpu''')
tf.app.flags.DEFINE_integer('batch_size', 32, '''the batch size''')
tf.app.flags.DEFINE_boolean('restore_model', False, '''if restore the pre_trained_model''')

tf.app.flags.DEFINE_string('train_log_dir', 'train_log',
                           '''directory wherer to write event logs''')
tf.app.flags.DEFINE_integer('max_training_iter', 100,
...
def train():
    ...
    for i in range(FLAGS.max_training_iter):
        ...
        _, loss_v, infer_v = sess.run([train_op, loss, infer],
                                      {data_ph: train_batch_data_v, label_ph: train_batch_label_v})
        ...
        if i % 500 == 0:
            ...
            test_loss_v, infer_v = sess.run([test_loss, infer],
                                            {data_ph: test_batch_data_v, label_ph: test_batch_label_v})
